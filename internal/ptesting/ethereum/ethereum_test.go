...
func TestHandleRecordStatus(t *testing.T) {
	...
	var txRecordList []*data.EvmTransactionRecord
	var txPendingRecordList []*data.EvmTransactionRecord
	utils.CopyProperties(txRecords, &txRecordList)
	...
	ptesting.RunTest(ptesting.Preparation{
		...
		BeforePrepare: func() {
			...
			_, err := data.EvmTransactionRecordRepoClient.Delete(context.Background(), biz.GetTableName(CHAIN_NAME), req)
			assert.NoError(t, err)
			total, err := data.EvmTransactionRecordRepoClient.BatchSaveOrUpdateSelective(nil, biz.GetTableName(CHAIN_NAME), txPendingRecordList)
			assert.NoError(t, err)
			assert.Equal(t, total, int64(3))
			req.ClientDataNotEmpty = true
		},
		Test: func(plat *platform.Bootstrap) {
			ethereum.HandleRecordStatus(CHAIN_NAME, txPendingRecordList)
			list, err := data.EvmTransactionRecordRepoClient.List(context.Background(), biz.GetTableName(CHAIN_NAME), req)
			assert.NoError(t, err)
			assert.NotNil(t, list)
			assert.Equal(t, len(list), 3)
			for _, asset := range list {
...
func TestHandleRecordStatus1(t *testing.T) {
	...
	ptesting.RunTest(ptesting.Preparation{
		...
		Test: func(plat *platform.Bootstrap) {
			...
			for _, asset := range list {
				if asset.TransactionHash == txPendingRecordList[0].TransactionHash {
					assert.Equal(t, asset.Status, biz.PENDING)
				} else if asset.TransactionHash == txPendingRecordList[1].TransactionHash {
					assert.Equal(t, asset.Status, biz.PENDING)
				} else if asset.TransactionHash == txRecordList[0].TransactionHash {
					assert.Equal(t, asset.Status, biz.SUCCESS)
...
func TestHandleRecordStatus3(t *testing.T) {
	...
	var txRecord1 *data.EvmTransactionRecord
	var txRecord2 *data.EvmTransactionRecord
	utils.CopyProperties(txRecords[0], &txRecord1)
	utils.CopyProperties(txRecords[0], &txRecord2)
	txRecord1.TransactionHash = txRecord1.TransactionHash[0:len(txRecord1.TransactionHash)-1] + "3"
	txRecord2.TransactionHash = txRecord2.TransactionHash[0:len(txRecord2.TransactionHash)-1] + "4"
	txRecordList := []*data.EvmTransactionRecord{txRecord1, txRecord2}
	now := time.Now().Unix() - 100
	for _, record := range txRecordList {
	...
	ptesting.RunTest(ptesting.Preparation{
		...
		BeforePrepare: func() {
			...
			_, err := data.EvmTransactionRecordRepoClient.Delete(context.Background(), biz.GetTableName(CHAIN_NAME), req)
			assert.NoError(t, err)
			total, err := data.EvmTransactionRecordRepoClient.BatchSaveOrUpdateSelective(nil, biz.GetTableName(CHAIN_NAME), txRecordList)
			assert.NoError(t, err)
			assert.Equal(t, total, int64(2))
			req.ClientDataNotEmpty = true
		},
		Test: func(plat *platform.Bootstrap) {
			ethereum.HandleRecordStatus(CHAIN_NAME, txRecordList)
			list, err := data.EvmTransactionRecordRepoClient.List(context.Background(), biz.GetTableName(CHAIN_NAME), req)
			assert.NoError(t, err)
			assert.NotNil(t, list)
			assert.Equal(t, len(list), 0)
