...
func loadHeightFromDB(chainName string) (*common.DBBlockRecord, error) {
	lastRecord, err := data.CkbTransactionRecordRepoClient.FindLast(nil, biz.GetTalbeName(chainName))
	if err != nil {
		return nil, err
	}
	if lastRecord == nil {
...
func (store *stateStore) LoadPendingTxs() (txs []*chain.Transaction, err error) {
	records, err := data.CkbTransactionRecordRepoClient.FindByStatus(nil, biz.GetTalbeName(store.chainName), biz.PENDING, biz.NO_STATUS)
	if err != nil {
...
func (store *stateStore) LoadBlockHash(height uint64) (string, error) {
	...
	if err != nil {
		if fmt.Sprintf("%s", err) == biz.REDIS_NIL_KEY {
			// 从数据库中查询指定块高的数据
			lastRecord, err := data.CkbTransactionRecordRepoClient.FindOneByBlockNumber(nil, biz.GetTalbeName(store.chainName), int(height))
