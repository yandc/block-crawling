...
func (h *txHandler) OnSealedTx(c chain.Clienter, tx *chain.Transaction) (err error) {
	...
	transactionInfo := tx.Raw.(*client.TransactionInfo)
	...
	if err != nil {
		log.Error(h.chainName+"扫块，从链上获取区块hash失败", zap.Any("current", curHeight) /*, zap.Any("new", h.chainHeight)*/, zap.Any("error", err))
		return
	}
		...
		if strings.HasPrefix(scriptFunction.Function, "peer_to_peer") {
			...
			if fromAddress != "" {
				...
				if err != nil && fmt.Sprintf("%s", err) != biz.REDIS_NIL_KEY {
					...
					alarmOpts := biz.WithMsgLevel("FATAL")
					biz.LarkClient.NotifyLark(alarmMsg, nil, nil, alarmOpts)
					log.Error(h.chainName+"扫块，从redis中获取用户地址失败", zap.Any("current", curHeight) /*, zap.Any("new", h.chainHeight)*/, zap.Any("error", err))
					return
				}
			}
			...
			if fromAddressExist || toAddressExist {
				...
				if txType == biz.TRANSFER {
					...
					if err != nil {
						...
						alarmOpts := biz.WithMsgLevel("FATAL")
						biz.LarkClient.NotifyLark(alarmMsg, nil, nil, alarmOpts)
						log.Error(h.chainName+"扫块，从nodeProxy中获取代币精度失败", zap.Any("current", curHeight) /*, zap.Any("new", h.chainHeight)*/, zap.Any("error", err))
					}
					tokenInfo.Amount = amount
				}
				stcMap := map[string]interface{}{
			...
			if err != nil {
				log.Error(h.chainName+"扫块，从链上获取区块event失败", zap.Any("current", curHeight) /*, zap.Any("new", h.chainHeight)*/, zap.Any("error", err))
				return err
			}
