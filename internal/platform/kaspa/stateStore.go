...
func loadHeightFromDB(chainName string) (*common.DBBlockRecord, error) {
	lastRecord, err := data.KasTransactionRecordRepoClient.FindLast(nil, biz.GetTableName(chainName))
	if err != nil {
		return nil, err
	}
	if lastRecord == nil {
...
func (store *stateStore) LoadPendingTxs() (txs []*chain.Transaction, err error) {
	records, err := data.KasTransactionRecordRepoClient.FindByStatus(nil, biz.GetTableName(store.chainName), biz.PENDING, biz.NO_STATUS)
	if err != nil {
