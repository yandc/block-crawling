...
func loadHeightFromDB(chainName string) (*common.DBBlockRecord, error) {
	lastRecord, err := data.AtomTransactionRecordRepoClient.FindLast(nil, biz.GetTalbeName(chainName))
	if err != nil {
		return nil, err
	}
	if lastRecord == nil {
...
func (store *stateStore) LoadPendingTxs() (txs []*chain.Transaction, err error) {
	records, err := data.AtomTransactionRecordRepoClient.FindByStatus(nil, biz.GetTalbeName(store.chainName), biz.PENDING, biz.NO_STATUS)
	if err != nil {
